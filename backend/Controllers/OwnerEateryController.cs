...
namespace backend.Controllers
{
    ...
    public class OwnerEateryController : ControllerBase
    {
        ...
        [HttpGet("eateries")]
        public async Task<IActionResult> GetEateriesByOwner()
        {
            // lấy id của owner từ token
            var ownerId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)!.Value);

            // kiểm tra xem id từ token là bao nhiêu
        ...
        public async Task<IActionResult> CreateEatery([FromBody] OwnerCreateEateryRequestDto request)
        {
            ...
            var ownerId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)!.Value);
            await _eateryRepo.CreateEateryAsync(ownerId, request);

            // dùng để kiểm tra dữ liệu từ frontend
            // Console.WriteLine($"OwnerId: {ownerId}");
            // Console.WriteLine($"CategoryId: {request.CategoryId}");
            // Console.WriteLine($"Name: {request.Name}");

            return Ok(new { message = "Tạo quán ăn thành công, đang chờ Admin duyệt!", id = ownerId });
        }
        ...
        public async Task<IActionResult> UpdateEatery(int id, [FromBody] OwnerCreateEateryRequestDto request)
        {
            ...
            var success = await _statsRepo.UpdateOwnerEateryAsync(id, ownerId, request);
