...
function Dashboard({ owner, onLogout }) {
    ...
    async function loadEateries() {
        try {
            const list = await ownerService.getEateriesByOwner(owner.ownerId)
            setEateries(list)
        } catch {
    ...
    const uploadFile = async (e, target) => {
        ...
        try {
            ...
            if (target === 'eatery') setForm(f => ({ ...f, imagePath: res.data.filePath }))
                        ...
                        <tr key={v.id} style={S.tr}>
                            <td style={S.td}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    {v.imagePath
                                        ? <img src={`https://localhost:7287${v.imagePath}`} style={S.thumb} />
                            ...
                            <td style={S.td}>
                                <span style={{ ...S.badge, ...(v.isOpenNow ? S.badgeOpen : S.badgeClosed) }}>
                                    {v.isOpenNow ? '● Đang mở' : '● Đã đóng'}
                                </span>
                            </td>
                ...
                <div style={S.menuGrid}>
                    {menuItems.map(item => (
                        <div key={item.id} style={S.menuCard}>
                            {item.imagePath
                                ? <img src={`https://localhost:7287${item.imagePath}`} style={S.menuImg} />
